 ...
 * 
 * Шаблон вида списка слайдов (отображается по умолчанию)
*/

defined('_JEXEC') or die;

JHtml::_('formbehavior.chosen', 'select'); // checkboxes
...
if ($saveOrder)
{
    ...
    // Drag & Drop
    /*
     * Метод загрузки скрипта сортировки html-таблицы
     * -'itemList' - id html-таблицы
     * -'adminForm' - name html-формы
     * $listDirn - направление сортировки
     * $saveOrderingUrl - URL для выполнения ajax-метода
     */
...
<form action="index.php?option=com_carousel&view=slides" method="POST" id="adminForm" name="adminForm">
    ...
    <table class="table table-striped table-hover" id="itemList">
        ...
        <tr>
            ...
            <th width="3%" ><?php echo JText::_('COM_CAROUSEL_ITEM_STATE'); ?></th>
            <th width="35%" ><?php echo JText::_('COM_CAROUSEL_CAPTION_LABEL'); ?></th>
            <th width="35%" ><?php echo JText::_('COM_CAROUSEL_IMAGE_LABEL'); ?></th>
            ...
            <?php if ( ! empty ($this->items) ) :?>
                ...
                <?php foreach ( $this->items as $item_num => $item ) : ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <!-- Выводим текст слайда ссылкой на соотв. слайд и обрезанием текста до 50 символов-->
